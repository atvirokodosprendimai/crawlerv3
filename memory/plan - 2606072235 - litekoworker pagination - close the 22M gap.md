...
- **2026-06-07 22:50** — Phase 1 executed (evidence-only, no code changes). All three code-level hypotheses (Total parse, off-by-one tail miss, pageButton block-2 navigation) **falsified** by direct evidence — see [[note - 2606072235 - liteko ground truth]]. Site baseline corrected from 22M to **2.235M**. Phases 2–4 no longer load-bearing for closing the gap; they remain correct fixes (defense-in-depth) but no longer urgent. The 130k-vs-2.2M gap is now attributed to **operational causes** — requires operator's prod DB to settle.
- **2026-06-07 23:15** — User granted SSH access (`ubuntu@dc-head`). Prod-DB probe pinpointed the real bug: `internal/app/service.go` line 216 was `_, _ = s.Frontier.Enqueue(...)` in a per-URL loop. Under litekoworker's `--batch 32 --concurrency 32` the SQLite writer pool (size 1) returned `SQLITE_BUSY_SNAPSHOT (517)` for most concurrent enqueues and the error was discarded silently. Distribution check on prod: 4,095 of 7,827 listings emitted only 1–20 detail URLs (classic lock-victim pattern); for 2024-03-15 only **11 of 50 page-1 case UUIDs** ever reached the frontier. Fix shipped in commit `4de0ef8`: added `frontier.EnqueueMany` and batched the whole discovered-links slice into one `WriteTX`; surfaced the inserted-vs-received gap via `slog.Warn`. Deploy + requeue runbook: [[note - 2606072315 - liteko requeue and redeploy]]. Plan goal now within reach — once redeployed and requeued, per-listing avg should rise from 16.4 toward ~200–400, hitting the original 2.235M target.
